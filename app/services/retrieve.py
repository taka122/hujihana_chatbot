...
def hybrid_retrieve(
    ...
    for chunk, raw_score, file_name, mime_type in vector_rows:
        cid = chunk.id
        score = vec_norm.get(cid, float(raw_score or 0.0))
        merged[cid] = _to_retrieved(chunk, file_name, mime_type, score, raw_score=float(raw_score or 0.0))

    for chunk, raw_score, file_name, mime_type in keyword_rows:
        cid = chunk.id
        ...
        if cid in merged:
            ...
            merged[cid] = _to_retrieved(chunk, file_name, mime_type, score, raw_score=raw_kw_score)
...
def _run_vector_search(
    ...
    stmt: Select[tuple[Chunk, float, str, str]] = (
        select(Chunk, score_expr, Document.file_name, Document.mime_type)
...
def _run_keyword_search(
    ...
    stmt: Select[tuple[Chunk, float, str, str]] = (
        select(Chunk, rank, Document.file_name, Document.mime_type)
        .join(Document, Document.id == Chunk.document_id)
        .where(Chunk.workspace_id == workspace_id)
        .where(Document.status == DocumentStatus.ready)
...
def _to_retrieved(chunk: Chunk, file_name: str, mime_type: str, score: float, raw_score: float) -> RetrievedChunk:
